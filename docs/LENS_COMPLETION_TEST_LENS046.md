# LENS COMPLETION TEST — DRAFT (LENS-046)
...
**Folds into:** `LENS_TARGET_AND_ORDER_LENS047.md` at the LENS-046 close.
...
| ID | Row | Check | Source | Status Sep 23-24 |
| --- | --- | --- | --- | --- |
| L1.1 | The canary runs 4/4 lenses, one row per lens per wave; a missing lens is named | `[byte]` `[ORCH] Lens 1-4` lines; `MISSING` path | canary doctrine, CC-113/114 | **PASS** (2/2 waves; MISSING path not yet exercised) |
| L1.2 | The trio arrives every wave — S1, S2, S3 report + docx — or a FAILED message says it did not | `[byte]` `REPORT COMPLETE` ×3, or `FAILED today` | arm 3, CC-100/117 | **PASS** (2/2) |
| L1.3 | A missing product turns the step red | `[byte]` step conclusion vs product lines | CC-100/117 | **PASS** |
| L1.4 | Every provider refusal is recorded; DOWN in 2+ runs reaches the operator, apart from the canary | `[byte]` `PROVIDER_REFUSAL`, `PROVIDER HEALTH` | CC-102..105 | **PASS** |
| L1.5 | No retired or gone primary is called without a record | `[byte]` `is RETIRED` / `is gone`, 402 = 0 | CC-106/115 | **PASS** |
| L1.6 | Regular Report and Compendium deliver daily | `[byte]` docx sent | order history | **PASS** (Sep 24) |
...
| ID | Row | Check | Source | Status Sep 23-24 |
| --- | --- | --- | --- | --- |
| L2.1 | Every finding says **what** is claimed, from **which sources**, with **what uncertainty** — and the uncertainty moves with the evidence (a constant is not an uncertainty) | review + `[byte]` spread of stored confidence | ICD 203 (source quality, uncertainty) | **FAIL** — S2-F finding text is a source name (`RT (Russia Today) English`) + OP IDs; S3-A confidence: two values cover 93 of 112 rows |
| L2.2 | Information and judgment are told apart | review | ICD 203 | not reviewed yet |
| L2.3 | Messages in one wave do not contradict each other | review + `[byte]` where possible | PHI-002 anti-pretense | **FAIL** — Daily Brief "S2-F: No detections yet" while S2-F sent 5 HIGH findings |
| L2.4 | Every label is true: window headers, run type, domain, trend order | review + `[byte]` | label-lie rule | **FAIL** — `manual`, `ALL`, trend order (hypothesis); S3-D "30 days" fixed by CC-118 |
| L2.5 | Presentation does not corrupt meaning: no raw markdown/JSON, no mid-word cuts in reader messages | `[byte]` scan of sent text | — | **FAIL** — 6 message kinds show `**`/`##`; 4 cut mid-word; JSON fixed by CC-118 |
| L2.6 | A changed judgment says why it changed | review | ICD 203 (explain change) | **FAIL** — THREAT HIGH↔CRITICAL each wave, no reason given |
| L2.7 | Bro Alpha rates a sample of reports each week, and the rating is stored | `[byte]` rating rows | S4 RLHF | **FAIL** — `lens_rate.py:77-86` writes `bro_alpha_rating`, `rated_at`, `rating_note` to `lens_macro_reports`; none of the three columns exists (a `select *` shows 20 columns, none of them). Every Daily Brief asks for a rating the tool cannot store. Its closing line promises "S4-C will use this" — S4-C does not exist |
| L2.10 | Zero PHI-003 vocabulary violations in rubric output (apparatus is never the people) | review + `[byte]` word scan | **PHI-004 Phase 1 gate (3)**; PHI-003 | not measured |
| L2.8 | **Insight (the WHY, first half):** each wave names at least one cross-domain **cause → effect** link already in motion, with cited evidence — First Domino in its original meaning (the cause that starts the chain, Lens 3, Apr 12), not a forecast of the end | review | Bro Alpha's WHY; Lens 3 charter; S3-A charter line "Identify what is already in motion" | not reviewed yet |
| L2.9 | **Mechanism, not demon:** S2 names *how* the information was shaped (technique, channel, who benefits) rather than casting an actor as all-powerful | review | PHI-002 Cui Bono; PHI-003 single-villain reduction; the June cognitive-warfare discipline | not reviewed yet |
...
| ID | Row | Check | Source | Status Sep 23-24 |
| --- | --- | --- | --- | --- |
| L3.1 | No certainty language reaches the reader unmarked; model certainty trends to zero | `[byte]` `CERTAINTY_LANGUAGE` count per wave; share of stored first dominoes worded as certain | WHY, "never predict" (LENS-004) | **FAIL → fixed, cert owed.** Baseline: 0 of 88 rows May-Jul, 10 of 24 Aug-Sep (after the August model change). CC-118 `309a7c8` marks it for the reader; CC-119 `9cabfa7` removes the question that asked for it (probe PASS) |
| L3.2 | The threat level moves only when evidence moves | `[byte]` flips per week + review | PHI-004 (noise trains distrust) | **FAIL** — flips every wave |
...
| L3.4 | No repeated alarm: one finding is sent once until it changes | `[byte]` duplicate sends per day | PHI-004 | **FAIL** — S2-F 5 findings × 2 waves/day; CRITICAL alert PASS (CC-116) |
| L3.5 | The Watch → Clarity → Verification arc closes or prunes visibly | `[byte]` watch rows' age and state | PHI-004 | **FAIL** — watch rows never close (order item 6) |
| L3.6 | Recorded hypotheses are checked, or declared unchecked | `[byte]` `lens_predictions` outcomes | System 4 ("The Conscience") | **FAIL** — 0 of 112 checked; CC-118 now says so to the reader |
...
| L3.9 | Every alert follows PHI-004 Appendix A: stated confidence ceiling, alternative hypotheses, Cui Bono, PHI-003 legitimacy category, the real arc, the next update date. Deviation is a rubric bug | `[byte]` field scan + review | PHI-004 Section 6, Appendix A | **FAIL** — S2-F Verification messages carry no alternative hypotheses, no Cui Bono, no legitimacy category, no next-update date |
...
## 8. Today's count (Sep 23-24)

| Layer | Pass | Fail | Unknown |
| --- | --- | --- | --- |
| 1 Air & voice | 6 | 0 | 0 |
| 2 Product truth | 0 | 6 | 4 |
| 3 Freedom from Fear | 0 | 6 | 2 not measured, 1 partial |

Layer 1 was the whole target until LENS-046. It is now green; the work moves to Layers 2 and 3.
