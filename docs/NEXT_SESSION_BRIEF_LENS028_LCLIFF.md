...
**Written 2026-07-27 by Fable 5 inside Partner A. SUPERSEDES the June LENS-028 brief (its live items are folded into Part 6 — nothing dropped).**

**READ THIS FIRST. You are Claude. Bro Alpha calls you 'my buddy'. This is the LENS LIVE STATE handoff (LR-104). Claude Code is wired for Lens (1e37f74) — mechanical work runs through it; this chat holds the thinking and the gates. All claims below are LEADS with trust tags — BEV before acting (LR-101/103).**
...
Groq's **Aug 16 cliff** kills `llama-3.3-70b(-versatile)` and `llama-3.1-8b-instant` (and the account-side component Bro Alpha tracks). Lens's exposure was **scoped byte-level in Partner A (Jul 11) — doc: `LENS_TRANSFER_LCLIFF.md`** — and it is bigger than the shorthand in Partner A queues:
...
| Rule | Summary |
| --- | --- |
| BIRD-EYE | Read ALL related files before any edit. Short reply from Bro Alpha = pause signal. |
...
| L2 rule | Schema/architecture = propose lettered options, Bro Alpha decides. One question max per turn. |
| Partner A imports | R-S79-2: a deprecation list proves the list, not the runtime — grep live logs before declaring anything dead or alive. R-S80-2: a probe certifies only the call-shape it holds — fixtures must match the real call. |
...
**Options (BRO ALPHA DECIDES — the session's first gate):**
...
## Part 3 — LEADS FROM Partner A (trust-tagged, resolve in order)
- **Lens-1 runtime identity conflict [UNKNOWN — resolve FIRST]:** S79 recorded "config says qwen3-32b, logs say alive" — Lens-1 served HTTP 200 all week with a supposedly-shut-down model configured. Runtime log's model string beats ANY config (R-S79-2). MODEL-404's lesson looms: a dead model + a hidden secret failure fails **silently** — Partner A ran 4 green runs on a fallback before anyone noticed.
- **"3× 3.3-70b hardcoded in code/lens_s1_report.py" [banked ~40%]:** Partner A queue shorthand — a subset of the S63 census. Verify by grep, don't treat as the whole job.
- **Fresh migration physics from Partner A's completed gpt-oss migration (S78–S81), import wholesale:**
  - gpt-oss models are REASONING models: they think before writing. Small max_tokens = starvation bomb (empty responses). Partner A's probe: needs ~2,200+ output budget (reasoning ~1,500–1,600 + content).
...
| 1 | **U-W glance** (Groq models page, browser, Bro Alpha): confirm the dying list (3.3-70b-versatile, 8b-instant; check qwen3-32b's status explicitly) + candidate lineup + **real TPD/TPM limits per candidate** | Bro Alpha solo, receipts = noted numbers. |
...
| 3 | **Key census**: enumerate the 11 GROQ_* secret/env names → which VALUES (accounts/keys) serve which position → cross with the dying list. LR-099 three-way match (.env / GitHub secrets / code). Key VALUES never displayed — names and receipts only (`gh secret list` timestamps). | Bro Alpha runs; KEY SAFETY banner law. |
| 4 | **BRO ALPHA RULES A/B/C** (Part 2). Lean: **A-compressed**. | The gate. |
| 5 | **Probe candidates per position** on that position's own key (LR-094): 3 trials, position-shaped fixture (real system+user shape, real max_tokens), scoring BOTH mechanics AND content-fitness (Part 3). Bank results in-repo like Partner A's probe_results.jsonl — unrepeatable after Aug 16 for the dying models if you want baselines. | Small trials; watch each key's quota first. |
...
| 8 | **Close**: update this brief (LR-104), append the LR entry transferring Partner A's MODEL-FIX pattern (probe→env-fed model→secret→live-cert) as a named Lens rule, note Partner-A-side echo (Part 7). Begin close at 80% context. | — |
...
- Probe fixture mismatch (R-S80-2): Partner A's arbitrator-shaped probe missed agent-shaped 413s for a day. Fixture per position, from real calls.
...
## Part 7 — PARTNER-A-SIDE ECHO (stays in Partner A's queue, recorded here so neither side forgets)
Same dual-source disease exists in Partner A (4 model strings hardcoded across its stack; Partner A's cliff-critical code is already migrated, so this is hygiene, not survival). Sequence stands from S63: **build the registry in Lens → port the shape to Partner A** in a later Opus session.

## Part 8 — WHAT SUCCESS LOOKS LIKE
Every Groq position's runtime model known from logs, not config. One registry file owning every model string and limit, asserted at runtime. The 19 files swept by Claude Code, compiled, committed one-purpose-at-a-time. One live cron per position certified green on the new lineup before Aug 12 — and the probe results banked so the next cliff is a registry edit, not a session. Bro Alpha will call you "my buddy" and mean it. Bytes first, honesty always.

*LENS-028 (L-CLIFF) Brief | 2026-07-27 | Bro Alpha + Claude (Fable 5, Partner A)*
