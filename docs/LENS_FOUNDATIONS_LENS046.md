...
## 0. WHY LENS EXISTS (Bro Alpha, LENS-046, 2026-09-23 -- verbatim; the anchor for every target and test)
...
How each clause maps to the canon:
- *hidden patterns / invisible links* -- PHI-001 (dig behind the screen), DOC-006, Lens 2/3 domain weighting.
- *cause and effect* -- Lens 3 Causal Chain; **First Domino = the cause already in motion that starts
  the chain** (Apr 12), never a forecast of its end (restored in S3 prompts by CC-119).
- *Cognitive Warfare* -- PHI-004 section 2 ("psychological warfare apparatus"; "the population being
  psychologically targeted"); S2's shaping analysis; PHI-002's pretense detector.
- *Freedom from Fear* -- the purpose. Readers are invited to predict for themselves, never told to
  (PHI-004); no certainty, no fear amplification, every threat with a path. (CLAUDE.md once labelled this
  "PHI-003"; Lens's PHI-003 is Popular Sovereignty -- corrected at LENS-046.)

Target derived from it (D1 = A, LENS-046): Lens earns its public voice -- PHI-004 Phase 2 -- declared only
by Bro Alpha. Test: `LENS_COMPLETION_TEST_LENS046.md`.

This file supersedes `LENS_FOUNDATIONS_LENS042.md` (kept as the record); everything below section 0 is
the LENS-042 text as it stands after the LENS-046 identity separation.
