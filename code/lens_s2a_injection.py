"""
lens_s2a_injection_v2.py — System 2 Position A: Injection Tracer
Project Lens | LENS-009
Model: llama-3.3-70b-versatile (Groq — GROQ_S2_API_KEY — dedicated S2 key)
UPDATED: switched to GROQ_S2_API_KEY for provider isolation
"""
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [S2-A] %(levelname)s %(message)s", datefmt="%H:%M:%S")
...
- EMOTIONAL_PRIME: emotionally loaded language placed before factual claims to bias interpretation
- SOURCE_LAUNDER: state media or anonymous claim cited as if from neutral/expert source
...
- If you find nothing suspicious, return empty findings — do NOT invent patterns
  ...
  "findings": [
    {
      ...
      "flagged_phrase": "<exact quoted text>",
      "confidence": <0.0-1.0>,
...
def call_injection_tracer(client: Groq, report: dict) -> Optional[dict]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            log.info(f"S2-A result for {lens_name}: {len(parsed.get('findings', []))} findings, score={parsed.get('overall_injection_score', 0)}")
            return parsed
...
def run_s2a(cycle: Optional[str] = None, run_id: Optional[str] = None) -> dict:
    start = time.time()
    if not run_id: run_id = f"s2a_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    log.info(f"=== S2-A Injection Tracer START | run_id={run_id} | cycle={cycle} ===")
    try:
        sb = get_supabase(); groq_client = get_groq()
    except Exception as e:
    ...
    summary = {"status": "COMPLETE", "run_id": run_id, "cycle": cycle, "reports_analyzed": len(reports), "reports_saved": saved_count, "total_findings": total_findings, "elapsed_seconds": elapsed, "results": results}
